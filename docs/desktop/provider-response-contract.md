# Tauri Provider Response Contract
...
User-facing timestamp rendering follows the shared rules in [../runtime/time-display.md](../runtime/time-display.md). The backend formats timestamps with `format_user_timestamp`; the frontend still runs display cleanup/local formatting where possible before rendering.
