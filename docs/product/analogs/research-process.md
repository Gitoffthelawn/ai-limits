# Analog Research Process

Note: "reference" is the working file naming convention. The research object may be a direct analog, a partial analog, or just a related product we can learn from. Do not assume market overlap or user overlap.

## Document hierarchy

This directory contains these document roles:

- **comparison.yaml** — quick overview across tools. Use it when you need status, short summaries, and fast comparison signals.
...
- **research-process.md** (this document) — research process. Use it only when conducting or updating research.

## 1. Discover

Sources: community mentions, Reddit, GitHub trending, product launches, user feedback.

Add a stub entry to `comparison.yaml` with `meta.status: not_researched`.

## 2. Docs review → `status: docs_reviewed`

Goal: answer the required `comparison.yaml` comparison questions from documentation:
...
If documentation reveals other relevant nuances, do not ignore them. Put them into the appropriate `{id}.md` sections and reflect only the short product relevance signal in `comparison.yaml` `summary`.

## 3. Hands-on → `status: hands_on`

Only if docs review leaves material uncertainty about a core feature.

Goal: verify the uncertain `comparison.yaml` comparison questions by running the product:
...
4. Record hands-on findings in the reference document.
   ...
   - Put additional relevant nuances into the appropriate `{id}.md` sections; keep `comparison.yaml` focused on `compare` (including `capabilities`) and `summary`.
...
- Update `compare` after research:
  ...
  - `capabilities` — array of verified feature tags from the closed vocabulary in `comparison.yaml` header comments (`usage`, `limits`, `session_limits`, `reset`, `credits`, `cost`, `burn_rate`, `forecast`, `usage_history`). Use `[]` when none are confirmed. List the union of what the tool exposes; put provider-specific gaps in `summary`.
