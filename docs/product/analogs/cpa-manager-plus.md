...
- Credits are ambiguous for the `comparison.yaml` closed vocabulary. The code parses Codex `credits`/`credits_balance` and displays rate-limit reset credits, but source review did not confirm a primary UI surface for a standalone prepaid/API credit balance. Therefore `credits` is not listed in `compare.capabilities`.
...
- No user-facing notification capability was confirmed in hands-on; `comparison.yaml` keeps `notifications: null`.
